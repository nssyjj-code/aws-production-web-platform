...
create_security_group() {
  ...
  aws_cli ec2 create-security-group \
    ...
    --tag-specifications "ResourceType=security-group,Tags=[{Key=Name,Value=$security_group_name},{Key=Project,Value=$PROJECT_NAME},{Key=Environment,Value=${ENVIRONMENT:-development}},{Key=ManagedBy,Value=aws-cli},{Key=Tier,Value=$tier}]" \
    --query "GroupId" \
    --output text
}
